...
const LatestJobs = () => {
  ...
  useEffect(() => {
    ...
    setLatestJobs(mockData);
  }, []); // Ensure this runs only once

  useEffect(() => {
    if (currentMode) {
        const filteredData = LatestJobs.filter((job) => job.mode === currentMode);
        setFilteredjobs(filteredData);
    } else {
      setFilteredjobs(LatestJobs);
    }
  }, [LatestJobs, currentMode]);
  ...
  const handleLinkClick = (index, mode, event) => {
    ...
    if (currentMode === mode) {
      setCurrentMode(null); // If the same link is clicked again, reset to display all jobs
    } else {
      setCurrentMode(mode);
    }
  };
  ...
  return (
    <div className="flex justify-evenly items-center flex-col p-10 bg-blue-50">
      <h1 className="text-4xl">Latest jobs on Internshala</h1>
      ...
      <div className="cards latest-jobs w-full flex flex-col justify-evenly items-center gap-[10px]"  >
        <h2 className="text-2xl mb-4">Latest jobs</h2>
